...
if [[ -z "${LR4J_HOME}" ]]; then
    echo "You must set the LR4J_HOME env var to point to the directory containing 'lr4j-record-1.0/lr4j-record-1.0.so'"
    exit
elif [ ! -f ${LR4J_HOME}/lr4j-record-1.0/lr4j-record-1.0.so ]; then
    echo "'${LR4J_HOME}/lr4j-record-1.0/lr4j-record-1.0.so' not found!"
    exit
fi
...
sleep 20
nohup java -XX:-Inline -XX:TieredStopAtLevel=1 -agentpath:${LR4J_HOME}/lr4j-record-1.0/lr4j-record-1.0.so -jar spring-petclinic-customers-service/target/*.jar --server.port=8081 --spring.profiles.active=chaos-monkey > target/customers-service.log 2>&1 &
...
nohup java -XX:-Inline -XX:TieredStopAtLevel=1 -agentpath:${LR4J_HOME}/lr4j-record-1.0/lr4j-record-1.0.so -jar spring-petclinic-api-gateway/target/*.jar --server.port=8080 --spring.profiles.active=chaos-monkey > target/gateway-service.log 2>&1 &
